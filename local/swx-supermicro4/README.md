...
6. Run docker-machine with the generic driver:
    ...
    docker-machine create -d generic --generic-ip-address 172.109.152.120 --generic-ssh-port 64022 --generic-engine-port 64376 --generic-ssh-key ${devops}/secrets/ssh/sofwerx --generic-ssh-user swxadmin --engine-storage-driver overlay2 swx-u-ub-supermicro4

If that fails, you may safely remove it and try it again:

    docker-machine rm -y swx-u-ub-supermicro4
...
8. Create a `.dm` file for the dm host to auto-switch when you cd to the directory:
    ...
    echo "swx-u-ub-supermicro4" > .dm

9. Setup environment variables for traefik:

    swx environment create swx-supermicro4
    swx environment set DNS_DOMAIN supermicro4.opswerx.org
...
10. Setup the dm2environment association between the dm host to the environment it is part of, to allow auto-switching to the environment when you cd to the directory:
    ...
    trousseau set dm2environment:swx-u-ub-supermicro4 swx-supermicro4
...
11. Setup environment variable for `docker-compose` to know which `.yml` file to use for this environment, and the ARCH of this environment:
    ...
    swx environment set COMPOSE_PROJECT_NAME swxsupermicro4
    swx environment set COMPOSE_FILE swx-supermicro4.yml
...
13. Add CNAME records in Route53 to point `*.supermicro4.devwerx.org` over to the static public IP
...
- Copy the `traefik:` service from another environment's `.yml` file into the `swx-supermicro4.yml` file (it is very generic).
