...
resource "aws_instance" "instance" {
  ...
  tags {
    Name = "swx-a-${var.Project}-${var.Lifecycle}${count.index}"
    Project = "${var.Project}"
    Lifecycle = "${var.Lifecycle}"
  }
...
/* Define nifi.project-environment.zone round-robin A records */
resource "aws_route53_record" "nifi-ipv4" {
  zone_id = "${data.aws_route53_zone.selected.zone_id}"
  name    = "nifi.${var.Lifecycle}.${var.dns_zone}"
  type    = "A"
  ttl     = "300"
  records = ["${join(",", aws_eip.instance.*.public_ip)}"]
}

/* Define nifi.project-environment.zone round-robin AAAA records */
resource "aws_route53_record" "nifi-ipv6" {
  zone_id = "${data.aws_route53_zone.selected.zone_id}"
  name    = "nifi.${var.Lifecycle}.${var.dns_zone}"
  type    = "AAAA"
  ttl     = "300"
...
output "docker-machine" {
  value = "docker-machine create -d generic --generic-ip-address ${join(",", aws_eip.instance.*.public_ip)} --generic-ssh-key $${devops}/secrets/ssh/sofwerx --generic-ssh-user ubuntu --engine-storage-driver overlay2 swx-a-${var.Project}-${var.Lifecycle}0"
}
