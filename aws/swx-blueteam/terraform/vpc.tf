...
output "docker-machine" {
  value = "docker-machine create -d generic --generic-ip-address ${join(",", aws_eip.instance.*.public_ip)} --generic-ssh-key $${devops}/secrets/ssh/sofwerx --generic-ssh-user ubuntu --engine-storage-driver overlay2 swx-a-${var.Lifecycle}-host0"
}
